...
import br.edu.ifspsaocarlos.sdm.kifurecorder.tests.TestCase;

public class TestsActivity extends Activity implements CameraBridgeViewBase.CvCameraViewListener2 {
        ...
        try {
            while ((line = reader.readLine()) != null) {
                if (line.contains("#")) {
                    ...
                    currentTestCase++;
                    casosDeTeste[currentTestCase - 1] = new TestCase();
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if (TestsActivity.viewMode == VIEW_MODE_RGBA) return rgba;

        if (TestsActivity.viewMode == VIEW_MODE_TEST && escolheuImagemTeste == false) return rgba;

        if (TestsActivity.viewMode == VIEW_MODE_TEST && jaProcessouAImagemDeTeste) {
            return imagemProcessada;
        }

        if (TestsActivity.viewMode == VIEW_MODE_AUTOMATIC_TEST) {
...
*/
    ...
    private Mat lerImagemDosResourcesAdaptada(int idDaImagemResource, Size tamanhoDaImagem) {
        Mat imagemLidaDeArquivo = new Mat();
        try {
            imagemLidaDeArquivo = Utils.loadResource(TestsActivity.this, idDaImagemResource);
